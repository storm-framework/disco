...
class NTC {
  ...
  constructor() {
    ...
    for (let i = 0; i < this.names.length; i++) {
      ...
      rgb = this.rgb(color);
      hsl = this.hsl(color);
      this.values.push([rgb[0], rgb[1], rgb[2], hsl[0], hsl[1], hsl[2]]);
    }
  }
  ...
  name(color: string): [string, string, boolean] {
    ...
    for (let i = 0; i < this.names.length; i++) {
      ...
      ndf1 =
        Math.pow(r - this.values[i][0], 2) +
        Math.pow(g - this.values[i][1], 2) +
        Math.pow(b - this.values[i][2], 2);
      ndf2 =
        Math.pow(h - this.values[i][3], 2) +
        Math.pow(s - this.values[i][4], 2) +
        Math.pow(l - this.values[i][5], 2);
      ndf = ndf1 + ndf2 * 2;
      if (df < 0 || df > ndf) {
        df = ndf;
        cl = i;
